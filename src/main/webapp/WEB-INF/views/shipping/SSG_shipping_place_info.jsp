		...
		<div class="paginate notranslate">
             <c:forEach var="pageNum" begin="1"	end="${ pdto.totalPages }">
            	...
            	<c:if test="${not (pageNum eq pdto.currentPage) }">
            		<a href="<%= request.getContextPath()%>/shippingPlace/list.do?currentPage=${ pageNum }">${ pageNum }</a>
            	</c:if>
            </c:forEach>
		</div>
...
<script>
	...
	function deleteBtn(element){
	    ...
	    //alert( spdtoId );
	    location.href = `/ShippingPlaceDelete.do?id=\${spdtoId}`;
	}
	...
	$("#editDefaultBtn").on("click", function(){
		...
		if( !defaultVal ){
			...
			//alert(defaultVal); 
			location.href = `/ShippingStatusEdit.do?id=\${defaultVal}&memid=<%= mid %>&status=기본배송지`
		}
	})
	...
	$("#editEbunmanBtn").on("click", function(){
		...
		if( !ebunmanVal ){
			...
			//alert(ebunmanVal);
			location.href = `/ShippingStatusEdit.do?id=\${ebunmanVal}&memid=<%= mid %>&status=이번만배송지`
		}
	})
